...
export default function RatingCard({
  ...
  const handleAvaliar = async (rating: number, texto: string) => {
    ...
    const responseAvaliar = await fetch(
      `http://localhost:3001/product-ratings/${ratingId}`,
      {
        method: "PATCH",
        headers: {
  ...
  const deleteAvaliar = async () => {
    ...
    const responseDeletar = await fetch(
      `http://localhost:3001/product-ratings/${ratingId}`,
      {
        method: "DELETE",
        headers: {
  ...
  return (
    <article className="bg-white dark:bg-[#151515] rounded-[28px] w-[400px] md:w-[600px] lg:w-[700px] shrink-0 min-h-[220px] flex items-start px-6 md:px-8 gap-4 md:gap-6 relative py-8 overflow-hidden transition-colors">
      ...
      <div className="flex flex-col gap-1 flex-1 items-start pt-2">
        <div className="flex items-center gap-3">
          <span className="text-black dark:text-white font-semibold text-3xl line-clamp-2">
            {name}
          </span>
        </div>

        <p className="font-spartan-light text-black dark:text-gray-300 text-base line-clamp-4">
          {text}
        </p>
      </div>
      ...
      <div className="flex flex-col items-end justify-between h-full pt-1 py-6">
          ...
          {isOwner && (
            <>
              <button
                ...
                className="w-8 h-8 bg-[#6B46C1] rounded-full flex items-center justify-center text-white hover:bg-purple-800 transition-colors shadow-sm"
        ...
        <span className="text-[#6A38F3] dark:text-[#B99CFF] text-sm cursor-pointer">
          ver mais
        </span>
      </div>
    </article>
